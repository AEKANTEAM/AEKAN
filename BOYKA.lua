...
if text == 'غادري'..(database:get(bot_id..'Name:Bot') or 'ايكان')..' غادر' then  
...
Text = '📮¦ عدد الجواهر التي ربحتها هي *» { '..Num..' } مجوهره *'
end
send(msg.chat_id_, msg.id_,Text) 
end
...
------------------------------------------------------------------------
local A7KANBOT = database:get(bot_id.."Add:Filter:Rp2"..text..result.chat_id_)   
if A7KANBOT then    
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
send(msg.chat_id_,0,"⚠¦العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛¦["..A7KANBOT.."] \n") 
